...
**4.2 MD5算法与MAC算法的区别**
> [!NOTE]
> MD5（哈希码）是用来生成固定长度的消息摘要（通常是128位）。它将任意长度的数据输入，通过复杂的算法处理，生成一个固定长度的输出，称为哈希值或消息摘要。用于确认数据的真实性和完整性：确保数据来自可信来源且未被篡改。
> MAC（消息认证码）用于验证消息的完整性和认证消息的来源。它结合了密钥和消息，通过加密算法生成一个固定长度的输出（MAC值）。通常用于通信协议中，确保消息在传输过程中未被篡改且来自合法发送者。
> MD5用于生成固定长度的哈希值，主要用于数据完整性校验。而MAC用于消息认证和完整性验证，通过结合密钥和输入消息生成认证码，提供更高的安全性。在需要认证功能的场合，通常会使用MAC而不是单纯的哈希函数。
